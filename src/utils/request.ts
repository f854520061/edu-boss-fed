...
// 响应拦截器
let isRreshing = false // 控制刷新 token 的状态
let requests: Array<Function> = [] // 存储刷新 token 期间过来的401请求
request.interceptors.response.use(function (response) { // 当状态码为2xx都会进入这里
  console.log('请求响应成功了', response)
  return response
}, async function (error) { // 超出2xx状态码都执行这里
  if (error.response) { // 请求发出去了收到响应了，但是状态码超出了2xx范围
